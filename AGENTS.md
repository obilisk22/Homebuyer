...
> Last updated: 2026-07-18 (assigned schools on Neighborhood — LAUSD GIS + SchoolDigger; Map Nearby schools panel removed)
...
| SchoolDigger ratings + parent-review enrich | `app/core/schooldigger.py` |
...
- [x] Assigned schools on Neighborhood (2026-07-18): **Elementary / Middle / High** cards resolved via **LAUSD attendance-boundary point-in-polygon** (`app/core/school_zones.py`; ArcGIS `LAUSD_Schools` MapServer layers 4/5/6) + name resolve via layer-0 school points inside each zone; optional **SchoolDigger** stars + parent-review enrich (`app/core/schooldigger.py`, `SCHOOLDIGGER_APP_ID`/`SCHOOLDIGGER_APP_KEY`); honest empty states (no pin / outside LAUSD / boundary gap / no SchoolDigger keys); moved off the event loop via `resolve_assigned_schools_job` (`ui_jobs.py`). Map **Nearby schools** panel removed — Schools layer toggle/markers/legend on the Map tab are unchanged.
...
| — | **Done** | Assigned schools (LAUSD GIS + SchoolDigger) on Neighborhood; Map Nearby schools panel removed |
...
4. **Optional env:** `GOOGLE_MAPS_API_KEY` (preferred geocoding + library grocery/shelter via Places Nearby Search when set); `CENSUS_API_KEY` (Map ACS tract choropleths + Financials ACS county tax estimate + buy-vs-rent rent-growth CAGR — required for those); `SOCRATA_APP_TOKEN` (optional, crime rate limits); `GEMINI_API_KEY` (Neighborhood AI + Financials commentary); `GEMINI_MODEL` / `GEMINI_FINANCIAL_MODEL` (see §6 / §6b); `SCHOOLDIGGER_APP_ID` / `SCHOOLDIGGER_APP_KEY` (optional — assigned-school ratings + parent reviews on Neighborhood, see §6e; free DEV/TEST key at https://developer.schooldigger.com/).
...
4. Map tab: dark CARTO basemap; fullscreen control near zoom; flood/zoning/wildfire/AQI/schools/sale-price/income/crime toggles (Schools → NCES markers + legend only, no Nearby-schools panel); Street View expansion below map (free svembed, open by default); no always-on Census tip until Income is toggled without a key  
...
6. Neighborhood tab resolves a name, deep-link buttons open Reddit/City-Data/Niche/Google; for an **LAUSD-pinned** home, **Assigned schools** shows three Elementary/Middle/High cards (names at minimum; stars + parent reviews when `SCHOOLDIGGER_APP_ID`/`SCHOOLDIGGER_APP_KEY` are set); Gemini overview + things-to-do buttons work when `GEMINI_API_KEY` is set  
