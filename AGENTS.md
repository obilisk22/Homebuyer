...
| **App URL** | http://127.0.0.1:8080 (browser) · optional native window via `--native` / packaged exe |
| **Desktop** | NiceGUI `native=True` (pywebview); freeze via `packaging/`; installer Inno Setup — see `docs/PACKAGING.md` |
...
| Photos Gemini overall blurb | `app/core/gemini_photos.py` |
...
| Redfin ZIP Active-market chip (homes_sold vs P75) | `app/core/market_activity.py` (+ `redfin_sales.py`) |
...
| Cache platform | `app/core/cache/` (disk JSON/gzip, memo, singleflight, SWR, quantize); `overlay_cache.py` re-exports |
...
**Property:** `address`, `zillow_url`, `list_price`, `beds`, `baths`, `sqft`, `hoa_fee`, `year_built`, `home_type`, `cooling`, `has_central_ac`, `townhome_position`, `city`, `state`, `zip_code`, `latitude`, `longitude`, `thumbnail_photo_id`, `thumbnail_locked`, `notes`, `nearby_signals`, `nearby_signals_at`, `permits_activity`, `permits_activity_at`, `broadband_status`, `broadband_at`, `market_activity`, `market_activity_at`, `neighborhood_name`, `neighborhood_source`, `neighborhood_override`, `neighborhood_notes`, `neighborhood_gemini`, `neighborhood_gemini_for`, `neighborhood_things_to_do`, `neighborhood_things_to_do_for`, `financial_gemini`, `financial_gemini_for`, `photos_gemini`, `photos_gemini_for`
...
- [x] Gallery UX: denser larger tiles; no per-photo Remove; Photos pin + Gemini overall take (import on add only)
- [x] TODO-051 Active market chip (2026-07-19): Redfin ZIP monthly `homes_sold` ≥ max(12, national P75); lime chip; persist + stale refresh
- [x] TODO-052 Center townhome chip (2026-07-19): Townhouse + explicit mid-row listing language; magenta soft-risk; no chip if uncertain
- [x] TODO-053 Photos Gemini blurb (2026-07-19): URL-context overall take; `photos_v1` cache; Ask/Regenerate via `run.io_bound`
- [x] TODO-054 Remove header Gemini insights (2026-07-19): bulk button + dead `ensure_gemini_insights*` removed; in-tab Ask/Regenerate only
...
- [x] Cleanup sweep #3 (2026-07-19): removed dead Map schools-list / FCC credential-ping / photo-path / quality-gate helpers; consolidated `normalize_city` + bbox into `geo.py`; docs aligned (broadband chip needs no key)
...
- [x] TODO-050 Buy-vs-rent OPEX inflation (2026-07-19): monthly **maintenance** + **utilities** grow with `rent_growth_pct` in the projection (same timing as rent); PITI stays flat; chart caption notes the inflate
...
| `TODO-050` | **Done** | Buy-vs-rent — utilities + maintenance scale with inflation (rent growth) |
| `TODO-051` | **Done** | Library icon — “Active market” (Redfin ZIP `homes_sold` ≥ max(12, P75)) |
| `TODO-052` | **Done** | Library icon — “Center townhome” (Townhouse + mid-row listing language) |
| `TODO-053` | **Done** | Photos tab — Gemini overall property blurb (Zillow URL context) |
| `TODO-054` | **Done** | Remove property-header “Gemini insights” bulk button (quota) |
...
5e. **Lazy property tabs:** `property_page` creates empty `tab_panel`s; each module `render` runs once on first select (`mounted` set). Default tab (Photos) mounts immediately via timer; Map / Neighborhood / Financials wait. Intentional side effects stay tab-scoped: Map auto-geocode when Map mounts; assigned schools when Neighborhood mounts; Financials PMMS/maint/utilities backfill only when Financials mounts (`ensure_financial_job` via `run.io_bound` — not on the UI thread at property open).
...
1. Library loads dark theme at http://127.0.0.1:8080 as a list of clickable home cards (stretch thumb, Akira street with compact `#unit` when present, quieter price, meta chips, quiet PITI + Growth captions when financials exist — amber &lt;3% / lime &gt;6%, nearby proximity icons that open Google Maps directions (home↔place) on click + no-central-AC risk chip when applicable at card bottom-right, ⋮ menu); Creato page chrome; Sort + collapsed Filter with active count; Export CSV/JSON; muted homes count next to the heading
2. Paste a `/homedetails/..._zpid/` URL → Add home → photos + address + beds/price/sqft/HOA/year/type/cooling appear; card click opens the property (Photos tab only until other tabs selected); menu Zillow/Delete don't navigate the card; delete confirms; Photos pin sets library thumb 
3. Map pins (even with `UNIT` in the slug) — Map tab mounts Leaflet + auto-geocode only when selected
...
5. Financials (mounts on first select; brief Loading… while PMMS/backfill runs off-thread): primary **Your deal** + rent/**Rent control**; **Loan** / **Ownership** (tax/insurance/HOA/maint/**utilities**) / **Advanced** behind expansions; `?` help + per-field revert; offer + down in dollars (amber warn if &lt;20%); Enter recalculates; add/refresh → list/HOA/tax/insurance/rent/appreciation/rent-growth/maint/utilities match listing or show ACS/state/FHFA/Zillow/age-blend/provider/Default captions; interest rate from Freddie Mac PMMS by term (Manual if edited); **Rent control** → 2%/yr; unchecked → ACS county rent CAGR or 3% default; hero monthly = PITI + maint + utils; charts = monthly mix pie + **Buy vs rent + invest (net worth)**; Gemini Ask / Regenerate when `GEMINI_API_KEY` is set
6. Neighborhood tab (mounts on first select) resolves a name, deep-link buttons open Reddit/City-Data/Niche/Google; for an **LAUSD-pinned** home, **Assigned schools** shows three Elementary/Middle/High cards (names, a free CA Dashboard color badge when the CDS lookup hits, and Dashboard/Niche link buttons — no keys needed); Gemini overview + things-to-do buttons work when `GEMINI_API_KEY` is set
7. Property header: muted **Edit listing details** below **Refresh listing details** (no header Gemini bulk button); nearby + risk/market chips bottom-right of `.hb-property-hero` when cached  
8. Photos tab: pin thumbnail + optional **Ask Gemini** overall take when `GEMINI_API_KEY` is set  
9. `.\.venv\Scripts\pytest.exe -q` passes  
