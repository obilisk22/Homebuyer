...
- Optional: `FCC_BDC_USERNAME` + `FCC_BDC_HASH` (from [broadbandmap.fcc.gov](https://broadbandmap.fcc.gov) → Manage API Access) to enable the library **no fixed broadband** risk chip. Without them the chip stays off (never false-alarms).
...
The `/` library page is a **list**, not a grid: each wide clickable card shows a photo that stretches to match the text column, a large Akira street address, quieter Creato list price, chips for beds/baths/sqft/`$`-per-sqft, quieter type/year chips, and amber HOA when monthly HOA is $400+. When Financials exist, a quiet **PITI**/mo caption and **Appr. N%/yr** (amber when under 3%) appear under the chips. When a home is geocoded, **nearby proximity icons** may appear on the **bottom-right of the library card** (not over the photo) — OSM Overpass for highway, transit, playground, grocery, and shelter; with `GOOGLE_MAPS_API_KEY` set, Google Places refines grocery and shelter matches (merged with OSM). Icons show only when within range (highway ≤800 ft, transit/grocery ≤0.5 mi, playground ≤0.75 mi, shelter ≤0.5 mi); magenta flags risks (highway/shelter), lime marks amenities; hover for distance and nearest name. Magenta listing-risk chips in that same row: **no Central AC** when the Zillow listing clearly lacks central air, and **no fixed broadband** when `FCC_BDC_*` credentials are set and the block reports zero fixed terrestrial service (DSL/cable alone does not flag). Homes in **Los Angeles / Seattle / Austin** may also show an amber **high permit activity** chip (≥8 structural/electrical/demolition-ish permits within ~0.25 mi in 24 months). Stale nearby / permit / broadband lookups refresh best-effort after the cards paint. Optional notes appear as a short teaser line. Page chrome (title, Add, Sort/Filter, empty states) uses the same Creato dark theme as the cards.
